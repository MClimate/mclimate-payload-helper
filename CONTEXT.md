...
- **2026-07-07** — PIR Mini `lux` out-of-range handling (`pirMiniPayloadParser.ts`). Clients were receiving large raw sentinel values (e.g. `65535`, `65532`). Now the raw 16-bit reading is interpreted: `0x0000`–`0xFFFA` → `lux` = number, `luxStatus: 'ok'`; `0xFFFF` → `lux: null`, `luxStatus: 'disabled'`; `0xFFFB`–`0xFFFE` → `lux: null`, `luxStatus: 'sensor_error'`. `PirMiniData.lux` type widened to `number | null`, new `luxStatus: 'ok' | 'disabled' | 'sensor_error'` field. Reverses the earlier numeric-only decision (2026-06-18). Decoder tests updated (all valid-lux cases now include `luxStatus: 'ok'`; disabled/error cases assert `lux: null` + status; added `0xFFFA` max-valid boundary test). PIR Mini suite 12/12 passing.
- **2026-07-08** — Audited all `PirMiniCommands` against the live docs (`.../pir-mini-device-communication-protocol` cheat-sheet + subpages). All 17 command codes, byte layouts, and ranges match except `setOccupancyTimeout` (`0x38`): docs state acceptable values `1...65535` (1-min resolution) but schema allowed `min(0)`. Fixed `PirMiniCommandSchemas.setOccupancyTimeout.timeout` to `z.number().int().min(1).max(65535)` in `schemas.ts`. General commands (`0x02/04/10/11/12/19/1B/1C/1D/A4`) are correctly inherited from `GeneralCommands`, not redefined. Note: the cheat-sheet has a doc typo (`0x42` listed twice, `0x43` for get counter-reset-flag); the detailed subpages correctly show `0x43`=set / `0x44`=get counter reset flag, which the code follows.
...
- **2026-08-10** — New device: **HT Mini** (`DeviceType.HTMini = 'ht_mini'`, per `ht-mini-device-protocol.pdf`). Followed `docs/how-to-add-new-device.md`. **Decoder** (`htMiniPayloadParser.ts`): 5-byte keep-alive (cmd `0x01`) — 10-bit temperature `(T[9:0]−400)/10` °C (byte 1 bits 1:0 = T[9:8], byte 2 = T[7:0]), `relativeHumidity = XX·100/256` %, `batteryVoltage = ((XX·2200/255)+1600)/1000` V (same formulas as PIR Mini); `commandsReadingHelper(hexData, 10, DeviceType.HTMini)` for command answers with appended keep-alive. **`commandsReadingHelper.ts`**: added `HTMini` to the PirMini branch of case `'22'` → `ledBrightness`; all other HT Mini GET responses (`0x04/12/19/1B/1D/A4`) use the existing generic cases. **Encoder** (`HTMiniCommands.ts`): extends `GeneralCommands` (inherits `0x02/04/10/11/12/19/1B/1C/1D/A4` + custom hex); adds only `setLedBrightness` (`0x21`, 0–100%), `getLedBrightness` (`0x22`), `restartDevice` (`0xA5`). **Schemas**: `HTMiniCommandSchemas` (spread of General + 3 device entries) + `HTMiniCommandTypes`, registered in `DeviceCommandSchemas`. Registered in `CommandBuilder` (`ht_mini`), exported from `src/index.ts`. 28 new tests (11 decoder incl. doc keep-alive example `0x01028880C8` → 24.8 °C / 50 % / 3.33 V, and doc response examples `0x19C6`→16.5 min, `0x1D020C`, `0x222F`→47, `0xA400`; 17 encoder incl. doc examples `0x020F`, `0x1101`, `0x1C0300`, join-retry default `0x78`). 369/369 passing. Note: HT Mini has **no** temperature/humidity compensation, external thermistor, or D2D — do not copy HTSensor commands to it; its downlink surface is exactly General + LED brightness + restart.
