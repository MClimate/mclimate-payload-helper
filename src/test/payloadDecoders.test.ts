...
describe('PirMini payload decoder', () => {
	...
	test('keepalive with light sensor error value (0xFFFC)', () => {
		expect(uplinkPayloadParser('01028880FFFCC80101AF', DeviceType.PirMini)).toStrictEqual({
			...
			relativeHumidity: 50,
			lux: 0,
			luxStatus: 'sensor_error',
			batteryVoltage: 3.33,
			occupied: true,
			pirTriggerCount: 431,
		})
	})
	test('keepalive with light sensor max valid value (0xFFFA)', () => {
		expect(uplinkPayloadParser('01028880FFFAC80101AF', DeviceType.PirMini)).toStrictEqual({
			sensorTemperature: 24.8,
			relativeHumidity: 50,
			lux: 65530,
			luxStatus: 'ok',
...
describe('MultiSensor payload decoder', () => {
	test('simple keepalive', () => {
		expect(uplinkPayloadParser('820F17CB2DA80D0600040A3A01DD03', DeviceType.MultiSensor)).toStrictEqual({
			...
			batteryVoltage: 3.51,
			pir: false,
			hallSensorEvent: false,
			staticAQIAccuracy: 1,
			powerSourceStatus: 1,
		})
	})
	test('keepalive with response of preloaded code', () => {
		expect(uplinkPayloadParser('1202820F17CB2DA80D0600040A3A01DD03', DeviceType.MultiSensor)).toStrictEqual({
			keepAliveTime: 2,
			averageTemperature: 45.9,
			minTemperature: 44.4,
			maxTemperature: 47,
